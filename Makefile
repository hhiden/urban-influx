BINARY ?= influx
APP_NAME = influx
DOCKER_LATEST_TAG = docker.io/hhiden/$(APP_NAME):latest
DOCKER_MASTER_TAG = docker.io/hhiden/$(APP_NAME):master
RELEASE_TAG ?= $(CIRCLE_TAG)
DOCKER_RELEASE_TAG = hhiden/$(APP_NAME):$(RELEASE_TAG)
...
.PHONY: build_linux
build_linux:
	env GOOS=linux GOARCH=amd64 go build -o $(BINARY) ./cmd/source/main.go

build_mac:
	env GOOS=darwin GOARCH=amd64 go build -o $(BINARY) ./cmd/source/main.go
